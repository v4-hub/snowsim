...
import { Wind, Snowflake, Trees, Eye, ChevronLeft, ChevronRight, Mountain, HelpCircle } from 'lucide-react';
...
export default function UIOverlay() {
  ...
  // Sticky edge toggle button (always visible on right edge of panel)
  const toggleButton = (
    <button
      onClick={() => setLeftPanelOpen(!leftPanelOpen)}
      className="fixed left-0 top-1/2 -translate-y-1/2 z-30 flex items-center justify-center w-6 h-16 rounded-r-lg bg-slate-800/95 border border-l-0 border-slate-700/60 text-slate-400 hover:text-white hover:bg-slate-700 transition-colors backdrop-blur-md shadow-lg"
      style={{ left: leftPanelOpen ? '320px' : '0px', transition: 'left 0.3s ease' }}
    >
    ...
    <>
      ...
      <div className="absolute top-0 left-0 w-80 h-full bg-slate-900/90 text-slate-200 p-6 overflow-y-auto flex flex-col gap-6 backdrop-blur-md border-r border-slate-700/50 shadow-2xl z-10">
        ...
        <div className="mt-auto pt-4 border-t border-slate-700/50">
          <a
            href="qa.html"
            className="flex items-center gap-2 px-3 py-2.5 rounded-lg bg-slate-800/50 border border-slate-700/50 text-slate-300 hover:text-white hover:bg-slate-700/60 hover:border-slate-600 transition-all text-sm"
          >
            <HelpCircle className="w-4 h-4 text-blue-400" />
            <span>FAQ — How does this work?</span>
          </a>
        </div>
